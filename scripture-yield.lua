if SY_LOADED and not _G.SY_DEBUG == true then
    -- error("Scripture Yield is already running!", 0)
    return
end

pcall(function() getgenv().SY_LOADED = true end)
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Scripture Yield is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Scripture Yield FE v" .. currentVersion
...
About.TextSize = 14
About.Text = "Plugins are .SY files and should be located in the 'workspace' folder of your exploit."
...
PluginsHint.Text = "Download plugins from the SY Discord (discord.gg/78ZuWSq)"
...
SYMouse = Players.LocalPlayer:GetMouse()
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with SY,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
...
function saves()
    if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
        local readSuccess, out = readfile("SY_FE.SY", true)
        if readSuccess then
            if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
                local success, response = pcall(function()
                    ...
                    if vtype(json.keepSY, "boolean") then KeepInfYield = json.keepSY else KeepInfYield = true end
                ...
                if not success then
                    ...
                    writefile("SY_FE.SY", defaults, true)
                    wait()
                    saves()
                end
            else
                writefile("SY_FE.SY", defaults, true)
                wait()
                local dReadSuccess, dOut = readfile("SY_FE.SY", true)
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("SY_FE.SY", HttpService:JSONEncode(update))
	end
end
...
SYMouse.KeyDown:Connect(function(Key)
	if (Key==prefix) then
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Scripture Yield Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
function selectPart()
	...
	local function HighlightPart()
		if selected.Adornee ~= SYMouse.Target then
			selectionBox.Adornee = SYMouse.Target
		else
			selectionBox.Adornee = nil
		end
	end
	ActivateHighlight = SYMouse.Move:Connect(HighlightPart)
	local function SelectPart()
		if SYMouse.Target ~= nil then
			selected.Adornee = SYMouse.Target
			Path.Text = getHierarchy(SYMouse.Target)
		end
	end
	ClickSelect = SYMouse.Button1Down:Connect(SelectPart)
end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Scripture Yield support server.'}
...
CMDs[#CMDs + 1] = {NAME = 'hideSY', DESC = 'Hides the main SY GUI'}
CMDs[#CMDs + 1] = {NAME = 'showSY / unhideSY', DESC = 'Shows SY again'}
CMDs[#CMDs + 1] = {NAME = 'keepSY', DESC = 'Auto execute SY when you teleport through servers'}
CMDs[#CMDs + 1] = {NAME = 'unkeepSY', DESC = 'Disable keepSY'}
CMDs[#CMDs + 1] = {NAME = 'togglekeepSY', DESC = 'Toggle keepSY'}
...
function checkTT()
	local t
	local guisAtPosition = COREGUI:GetGuiObjectsAtPosition(SYMouse.X, SYMouse.Y)
	...
	if t ~= nil and t:GetAttribute("Title") ~= nil then
		...
		local xP
		local yP
		if SYMouse.X > 200 then
			xP = x - 201
		else
			xP = x + 21
		end
		if SYMouse.Y > (SYMouse.ViewSizeY-96) then
			yP = y - 97
		else
			yP = y
		end
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.SY_DEBUG then
...
local MousePositionToVector2 = function()
	return Vector2.new(SYMouse.X, SYMouse.Y)
end
...
local function clicktpFunc()
	pcall(function()
		...
		local hitPosition = SYMouse.Hit.Position
...
SYMouse.Button1Down:Connect(function()
	for i,v in pairs(binds) do
		...
		elseif v.COMMAND == 'clickdel' then
			...
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function() SYMouse.Target:Destroy() end)
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'SY_fe.SY' or name == 'SY_fe' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-3) == '.SY' then
			pcall(function() file = readfile(name) end)
			fileName = name
		else
			pcall(function() file = readfile(name..'.SY') end)
			fileName = name..'.SY'
		end
		if file then
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeSY/infiniteyield/master/source'))()")
	end
end)
...
addcmd('keepSY', {}, function(args, speaker)
...
addcmd('unkeepSY', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		updatesaves()
	else
...
addcmd('togglekeepSY', {}, function(args, speaker)
	if queueteleport then
...
function sFLY(vfly)
	...
	flyKeyDown = SYMouse.KeyDown:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = (vfly and vehicleflyspeed or SYflyspeed)
		elseif KEY:lower() == 's' then
			CONTROL.B = - (vfly and vehicleflyspeed or SYflyspeed)
		elseif KEY:lower() == 'a' then
			CONTROL.L = - (vfly and vehicleflyspeed or SYflyspeed)
		elseif KEY:lower() == 'd' then 
			CONTROL.R = (vfly and vehicleflyspeed or SYflyspeed)
		elseif QEfly and KEY:lower() == 'e' then
			CONTROL.Q = (vfly and vehicleflyspeed or SYflyspeed)*2
		elseif QEfly and KEY:lower() == 'q' then
			CONTROL.E = -(vfly and vehicleflyspeed or SYflyspeed)*2
		end
		pcall(function() workspace.CurrentCamera.CameraType = Enum.CameraType.Track end)
	end)
	flyKeyUp = SYMouse.KeyUp:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = 0
		elseif KEY:lower() == 's' then
			CONTROL.B = 0
	...
	mfly2 = RunService.RenderStepped:Connect(function()
		...
		if speaker.Character:FindFirstChildWhichIsA("Humanoid") and root and root:FindFirstChild(velocityHandlerName) and root:FindFirstChild(gyroHandlerName) then
			...
			if direction.X > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or SYflyspeed) * 50))
			end
			if direction.X < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or SYflyspeed) * 50))
			end
			if direction.Z > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or SYflyspeed) * 50))
			end
			if direction.Z < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or SYflyspeed) * 50))
			end
...
addcmd('float', {'platform'},function(args, speaker)
	...
	if pchar and not pchar:FindFirstChild(floatName) then
		task.spawn(function()
			...
			qUp = SYMouse.KeyUp:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue + 0.5
				end
			end)
			eUp = SYMouse.KeyUp:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue - 1.5
				end
			end)
			qDown = SYMouse.KeyDown:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue - 0.5
				end
			end)
			eDown = SYMouse.KeyDown:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue + 1.5
				end
			end)
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = Players.LocalPlayer.PlayerGui:GetGuiObjectsAtPosition(SYMouse.X, SYMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
addcmd('hideSY',{},function(args, speaker)
	isHidden = true
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('SY Hidden','You can press the prefix key to access the command bar') end
end)

addcmd('showSY',{'unhideSY'},function(args, speaker)
	isHidden = false
	minimizeNum = -20
	if wasStayOpen then
		maximizeHolder()
...
addcmd('tptool', {'teleporttool'}, function(args, speaker)
	...
	TpTool.Activated:Connect(function()
		...
		HRP.CFrame = CFrame.new(SYMouse.Hit.X, SYMouse.Hit.Y + 3, SYMouse.Hit.Z, select(4, HRP.CFrame:components()))
	end)
end)
...
addcmd('hovername',{},function(args, speaker)
	...
	nbUpdateFunc = SYMouse.Move:Connect(updateNameBox)
end)
...
addcmd("addallplugins", {"loadallplugins"}, function(args, speaker)
    ...
    for _, filePath in ipairs(listfiles("")) do
        local fileName = filePath:match("([^/\\]+%.SY)$")

        if fileName and
            fileName:lower() ~= "SY_fe.SY" and
...
task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet('https://raw.githubusercontent.com/EdgeSY/infiniteyield/master/version')
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Scripture Yield may have issues or features that are not functioning correctly.") end
end)
